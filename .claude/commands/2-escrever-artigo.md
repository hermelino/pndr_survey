...
- **Não iniciar parágrafo com citação:** Parágrafos nunca devem iniciar com `\citeonline{...}` ou `\cite{...}`. O argumento autoral deve preceder a evidência bibliográfica.
- **Preferir:** Voz passiva sintética quando adequada; frases diretas e objetivas
- **Travessões (`---`):** Usar com parcimônia. Preferir estruturas alternativas: orações subordinadas (com vírgulas ou dois-pontos), apostos entre vírgulas ou frases separadas. Reservar o travessão para inserções que realmente exijam destaque ou ruptura sintática. **Limite:** no máximo 1 par de travessões por parágrafo e 3 por subseção.
...
- **Extensão de parágrafos:** Limite parágrafos a 5-6 frases ou 150 palavras. Parágrafos excessivamente longos (>10 linhas no PDF) devem ser divididos em unidades menores, cada uma com um subtema claro. **Mínimo:** Parágrafos não devem conter menos de 2 sentenças. Parágrafos-frase (única sentença) devem ser fundidos com o parágrafo adjacente ou expandidos. **Verificação obrigatória:** Após redigir cada subseção, verificar que nenhum parágrafo excede 150 palavras nem contém menos de 2 sentenças antes de apresentar ao usuário.
- **Dados quantitativos com fonte:** Valores em R\$, percentuais e contagens devem sempre ter fonte indicada: citação bibliográfica formal `\cite{}`, referência a tabela do artigo `Tabela~\ref{}` ou nota de rodapé `\footnote{}` com identificação precisa do documento-fonte (nome, ano, URL). Nunca apresentar dados quantitativos sem atribuição. **Expressões genéricas como "segundo dados de X" ou "dados de X mostram que" NÃO constituem atribuição adequada** — é necessário citar o documento específico.
