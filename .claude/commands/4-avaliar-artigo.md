...
- **Arquivo principal:** `latex/0-main.tex` (inclui seções via `\input{}`)
...
- **Seções do artigo:**
  ...
  - Método (`sec:metodo` → `3-metodo.tex`)
...
| `metodo` | Seção Método completa (`3-metodo.tex`) |
...
1. Ler `latex/0-main.tex` para compreender a estrutura geral do documento.
...
1. **NUNCA** editar `0-main.tex`, `3-metodo.tex` ou qualquer arquivo `.tex` do artigo
